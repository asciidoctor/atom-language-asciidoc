describe 'Example inline paragraph', ->
  grammar = null
  ...
  describe 'Should tokenizes when', ->
    ...
    it 'contains simple phrase', ->
      ...
      expect(tokens[1][0]).toEqualJson value: 'A multi-line example.', scopes: ['source.asciidoc', 'markup.block.example.asciidoc']
  ...
  describe 'Should not tokenizes when', ->
    ...
    it 'beginning with space', ->
      ...
      expect(tokens[1]).toHaveLength 1
      expect(tokens[1][0]).toEqualJson value: 'A multi-line example.', scopes: ['source.asciidoc']
