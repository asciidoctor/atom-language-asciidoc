...
As the AsciiDoc syntax has changed quite a bit since the original Python implementation, Asciidoctor offers a legacy mode called `compat-mode`.
This mode can be set using the `:compat-mode:` attribute.
This can be done in the document iteself, or when generating the final result using `asciidoctor -a compat-mode`, or it's shorter equivalent `asciidoc`.
